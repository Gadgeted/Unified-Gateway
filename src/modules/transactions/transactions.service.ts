...
export class TransactionsService {
  ...
  async processIncomingPayment(dto: CreatePaymentDto, apiKey: string) {
    ...
    // Notice we capture the merchantReference directly from your POS tracking engine context
  ...
  // 👈 NEW: Webhook Callback Worker to resolve perpetual PENDING issues
  async updateTransactionByGatewayRef(checkoutRequestId: string, finalStatus: 'SUCCESS' | 'FAILED') {
    ...
    // Look up transaction using the unique tracking reference assigned by the payment provider
    ...
    return this.prisma.transaction.update({
      where: { id: transaction.id },
      data: { status: finalStatus === 'SUCCESS' ? TransactionStatus.SUCCESS : TransactionStatus.FAILED },
    });
  }

  // 👈 NEW: POS Shared Reference Query Mapping Anchor
  async getTransactionStatusByMerchantRef(merchantRef: string, apiKey: string) {
    // 1. Authorize connection client integrity matching
    const merchant = await this.prisma.merchant.findUnique({
      where: { apiKey },
    });
    if (!merchant) throw new UnauthorizedException('Access denied.');

    // 2. Fetch transaction using the shared matching invoice key generated by the POS
    const transaction = await this.prisma.transaction.findFirst({
      where: { 
